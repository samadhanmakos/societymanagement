...
//http://localhost/society_mgt/api/auditreport.php?title=audit1&attachment=17-180721105052-Alax3
  ...
  if(isset($_REQUEST['title']) && !empty($_REQUEST['title']) && !empty($_FILES['attachment']['name']) && isset($_FILES['attachment']['name'])) {
    ...
    $response = array();
  	...
  	if(move_uploaded_file($file_tmp, $target_dir.$attachment)){
  		...
  		$insert = insertToDb("tbl_audit", $arr,$message='Inserted');
    if($insert){
		echo json(array('status'=>'1','message'=>'Audit Report uploaded succesfully'));
		exit;
	}else
	{
